...
def tree_denoising(df_location,output_folder, index, libraryname,tolerance1='40ppm', tolerance2 = '20ppm',typeofmsms = 'msms'):
...
#         tolerance = '20
    ...
    try:
        p = subprocess.run(["sirius", "-o", 'sirius_workspace/%s/%s%s%s%s' % (output_folder,str(instance.row_num),instance.key,libraryname,tolerance),
                        "-f", instance.Formula, 
...
#         tolerance = '20
    ...
    if os.path.isdir('sirius_workspace/%s/%s%s%s%s/0_unknown_/spectra/' % (output_folder,str(instance.row_num),instance.key,libraryname,tolerance)):
        for file in os.listdir('sirius_workspace/%s/%s%s%s%s/0_unknown_/spectra/'%(output_folder,str(instance.row_num),instance.key,libraryname, tolerance)):
            if file.endswith('.tsv'):
                data_denoised = pd.read_csv("sirius_workspace/%s/%s%s%s%s/0_unknown_/spectra/%s" % (output_folder,str(instance.row_num),instance.key,libraryname,tolerance,file), 
                             sep = "\t")
                sta = pd.read_csv("sirius_workspace/%s/%s%s%s%s/formula_identifications.tsv" % (output_folder, str(instance.row_num),instance.key,libraryname,tolerance), 
                             sep = "\t")
                break
...
#         os.rename(src, dst)
        return(so.pack_spectra(mz, intensity), so.pack_spectra(mz_exact, intensity),float(sta['explainedIntensity']), float(sta['TreeScore']))
#         return(mz, intensity)
    else:
